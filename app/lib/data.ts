...
export async function fetchFilteredCustomers(query: string) {
  try {
    query = `%${query}%`;
    type customerInvoiceSQL = s.customers.SQL | s.invoices.SQL;
    const data = await db.sql<customerInvoiceSQL, CustomersTableType[]>`
		SELECT
		  customers.id,
		  customers.name,
		  customers.email,
		...
		WHERE
		  customers.name ILIKE ${db.param(query)} OR
        customers.email ILIKE ${db.param(query)}
